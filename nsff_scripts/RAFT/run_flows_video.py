...
def generate_motion_segmentation(basedir, threshold):
    ...
    for i in range(0, num_frames): #len(perm) - 1):
        ...
        # load optical flow 
        if i == 0:
          fwd_flow = read_optical_flow(basedir, im_ref.name, read_fwd=True)
          bwd_flow = np.zeros_like(fwd_flow)
        elif i == num_frames - 1:
          bwd_flow = read_optical_flow(basedir, im_ref.name, read_fwd=False)
        ...
        fwd_e_dist = compute_epipolar_distance(T_ref2post, K, p_ref_h, p_post_h)
        ...
        bwd_e_dist = compute_epipolar_distance(T_ref2prev, K, p_ref_h, p_prev_h)
    ...
    img_path_list = sorted(glob.glob(os.path.join(img_dir, '*.jpg'))) + sorted(glob.glob(os.path.join(img_dir, '*.png')))
...
def load_image(imfile):
    ...
    img = cv2.resize(img, (INPUT_W, int(round(float(img.shape[0])/ float(img.shape[1]) * INPUT_W))), cv2.INTER_LINEAR)
...
def compute_fwdbwd_mask(fwd_flow, bwd_flow):
    ...
    fwd_mask = fwd_lr_error < alpha_1  * (np.linalg.norm(fwd_flow, axis=-1) + np.linalg.norm(bwd2fwd_flow, axis=-1)) + alpha_2

    fwd2bwd_flow = warp_flow(fwd_flow, bwd_flow)
    bwd_lr_error = np.linalg.norm(bwd_flow + fwd2bwd_flow, axis=-1)

    bwd_mask = bwd_lr_error < alpha_1  * (np.linalg.norm(bwd_flow, axis=-1) + np.linalg.norm(fwd2bwd_flow, axis=-1)) + alpha_2
    ...
    with torch.no_grad():
        ...
        for i in range(images.shape[0]-1):
            ...
            img1 = cv2.resize(np.uint8(np.clip(image1[0].cpu().numpy().transpose(1, 2, 0), 0, 255)), 
                            (img_train.shape[1], img_train.shape[0]), cv2.INTER_LINEAR)
            img2 = cv2.resize(np.uint8(np.clip(image2[0].cpu().numpy().transpose(1, 2, 0), 0, 255)), 
                            (img_train.shape[1], img_train.shape[0]), cv2.INTER_LINEAR)

            fwd_flow = resize_flow(flow_up_fwd, img_train.shape[0], img_train.shape[1])
            # fwd_flow = refinement_flow(fwd_flow, img1, img2)

            bwd_flow = resize_flow(flow_up_bwd, img_train.shape[0], img_train.shape[1])
